...
targetStore = cat(2,diag(ones(100,1)),zeros(100,100));
...
while (epochs < maxEpochs)
    %Create a random permutation from 1 to 200 to select order of training
    ...
    for i = 1:200
        ...
        inputPattern = nan(100,1);
        
        %initialize the input pattern (selects a new target each i loop)
        for j = 1:100
            if(targetStore(j,thePermutation(i)) == 0)
                inputPattern(j,1) = rand*inputNoiseLevel;
            elseif(targetStore(j,thePermutation(i)) == 1)
                inputPattern(j,1) = rand;
            end
        end
        
        %Select target vector for error calculation
        targetVectorFoN = targetStore(:,thePermutation(i));
        ...
        output_error = targetVectorFoN - output_activation;
        ...
        if ((stimulusPresent == true && thePermutation(i) <= 100) || (stimulusPresent == false && thePermutation(i) > 100))
           FoNIsCorrect = true;
           tempPerformanceStoreFoN(i,1) = 1;
        elseif  ((stimulusPresent == true && thePermutation(i) > 100) || (stimulusPresent == false && thePermutation(i) <= 100))
           FoNIsCorrect = false;
        ...
        output_error_2 = targetVectorSoN - output_activation_2;
    ...
    %--SoN--   
    %Store the SoN proportion correct in this epoch into the performance
    %store
    ...
    %[COMMENTED OUT BECAUSE NOT NECESSARY (only used to calculate error)]
    %{
    %----------------------------------------
    % ---------------Outer FoN---------------
    %----------------------------------------
...
for i = 1:200
        ...
        elseif (thePermutation(i)> 100  && stimulusPresent == true)
            disp(string('   -- This assessment is incorrect.'));
            FoNIsCorrect = false;
       ...
       %output_activation_store(100,200) = 1;
        ...
        if (compMatrixLesioned == 0)
            compMatrix = inputPattern - output_activation;
        elseif (compMatrixLesioned == 1)
            compMatrix = inputPattern(21:100,1) - output_activation;
        end
